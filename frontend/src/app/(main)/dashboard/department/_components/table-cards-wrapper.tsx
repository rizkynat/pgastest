...
import CreateDepartmentModal from "./create-modal";
...
export function TableCardsWrapper({
  ...
  const [meta, setMeta] = React.useState<DepartmentsMeta | null>(null);
    ...
    async (pageIndex: number, pageSize: number) => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/departments` + `/search` + `?${params.toString()}`,
          {
            credentials: "include",
            cache: "no-store",
          }
        ...
        const json = (await res.json()) as DepartmentsResponse<any>;
  ...
  // ===============================
  React.useEffect(() => {
    fetchDepartments(pagination.pageIndex, pagination.pageSize);
  }, [fetchDepartments, pagination.pageIndex, pagination.pageSize]);
  ...
  const handleDelete = React.useCallback(async (row: any) => {
    const confirmDelete = confirm(`Apakah yakin ingin menghapus data ${row.department_name}?`);
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/departments/${raw.id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  const handleExport = React.useCallback(
    async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/departments/export/xlsx`,
        {
          method: 'GET',
          credentials: "include",
          cache: "no-store",
