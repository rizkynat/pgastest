...
export function TableCardsWrapper({
    ...
    async (pageIndex: number, pageSize: number) => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/employees` + `/search` + `?${params.toString()}`,
          {
            credentials: "include",
            cache: "no-store",
          }
  ...
  // ===============================
  React.useEffect(() => {
    fetchEmployees(pagination.pageIndex, pagination.pageSize);
  }, [fetchEmployees, pagination.pageIndex, pagination.pageSize]);
  ...
  const handleDelete = React.useCallback(async (row: any) => {
    const confirmDelete = confirm(`Apakah yakin ingin menghapus data ${row.employee_name}?`);
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/employees/${raw.id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  const handleExport = React.useCallback(
    async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/employees/export/xlsx`,
        {
          method: 'GET',
          credentials: "include",
          cache: "no-store",
