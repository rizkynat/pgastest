...
import CreateSpendingModal from "./create-modal";
...
export function TableCardsWrapper({
  ...
  const [meta, setMeta] = React.useState<SpendingsMeta | null>(null);
  ...
  // FETCH DATA
  // ===============================
  const fetchSpendings = React.useCallback(
    async (pageIndex: number, pageSize: number) => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/spendings` + `/search` + `?${params.toString()}`,
          {
            credentials: "include",
            cache: "no-store",
          }
        ...
        const json = (await res.json()) as SpendingsResponse<any>;
  ...
  // ===============================
  React.useEffect(() => {
    fetchSpendings(pagination.pageIndex, pagination.pageSize);
  }, [fetchSpendings, pagination.pageIndex, pagination.pageSize]);
  ...
  const handleDelete = React.useCallback(async (row: any) => {
    const confirmDelete = confirm(
      `Apakah yakin ingin menghapus data spending karyawan #${row.employee_id} tanggal ${row.spending_date}?`
    );
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/spendings/${raw.id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  const handleExport = React.useCallback(
    async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/spendings/export/xlsx`,
        {
          method: 'GET',
          credentials: "include",
          cache: "no-store",
